...
], function (MessageToast, History, formatter, Fragment) {
  'use strict';
  ...
  return {
    ...
    onApprove: function (oEvent) {
      ...
      oPromise.then(function (aResponse) {
        ...
        if (Array.isArray(aResponse) && aResponse.length > 0) {
          const response = aResponse[0].response;
        ...
        this.getView().setBusy(false);
        sap.m.MessageToast.show("Submission failed, please try again.");
    ...
    onReject: function (oEvent) {
      if (!this._pDialog) {
        this._pDialog = this.loadFragment({
            name: "zmytask.mytask.ext.fragment.Rejct"
    ...
    // _onPressreject: function(oEvent){
    onOkPress:function(){
       //REJECT FUNCTION
       this.getView().setBusy(true);
       const oHistory = History.getInstance();
       ...
       oPromise.then(function (aResponse) {
         debugger; 
         var successMessage = "Request Rejected";
         sap.m.MessageBox.show(successMessage, {
          icon: sap.m.MessageBox.Icon.SUCCESS,
    ...
    onvaluehelp:function(){
      if (!this._RDialog) {
        this._RDialog = this.loadFragment({
            name: "zmytask.mytask.ext.fragment.Rejectvaluehelp"
        }).then(function (oDialog) {
    ...
    this._RDialog.then(function (oDialog) {
        oDialog.open();
    });
